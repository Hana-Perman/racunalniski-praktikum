\begin{frame}{Logika in množice}
	\begin{enumerate}
		\item
		Poišči preneksno obliko formule \(\exists x : P(x) \land 
		\forall x: Q(x) \Rightarrow \forall x : R(x)\)
		\item 
		Definiramo množici \(A=[2,5] \) in 
		\(B=\{0,1,2,3,4,\ldots \}. \) 
		V ravnino nariši:
		\begin{enumerate}
		   \item \(A \cap B \times \empty\)
...
\begin{frame}{Analiza}
	\begin{enumerate}
		...
		$$ 
		\item
		Pokaži, da ima \(sin(2x)\) inverzno funkcijo in izračunaj 
		\((f^{-1})'(3\pi)\).
