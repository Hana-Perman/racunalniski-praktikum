...
\begin{frame}{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
	...
	\coordinate(T') at ({-\td}, {-\tt});
	 % Lok je definiran s točko, v kateri se lok začne (ne središče!),
	 % z začetnim in končnim kotom ter radijem.
	 % Koti so vedno podani enako: kot 0 je v smeri x osi in se veča
	 % v nasprotni smeri urinega kazalc
	...
	\coordinate(T'') at ({\td},{-\tt});
	\draw[gray, thin] (T'') arc[start angle=240, end angle=120, radius=2] node[right]{};
	% % Naloga 3.4.4.: Definirajte in narišite točko 
	\coordinate [label=below right:{\scriptsize{\(C\)}}] (C) at (0,{-\t});
	\draw (C) circle (1.5pt);
	\pause 
	% % Naloga 3.4.5.: Narišite premico skozi točki T in 
